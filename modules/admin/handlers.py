...
from services.messages_manage import broadcast_message, send_sticker_and_message_with_cleanup
...
async def admin_menu(message: types.Message):
    buttons = [
        types.InlineKeyboardButton(
            text=AdminMessages.CHECK_REQUESTS_BUTTON, callback_data="check_requests"
        ),
        types.InlineKeyboardButton(
            text=AdminMessages.DELETE_USER_BUTTON, callback_data="delete_user"
        ),
        types.InlineKeyboardButton(
            text=AdminMessages.BROADCAST_BUTTON, callback_data="broadcast"
        ),
        types.InlineKeyboardButton(
            text=AdminMessages.GET_USERS_BUTTON, callback_data="get_users"
        ),
        types.InlineKeyboardButton(
            text=AdminMessages.PROMO_CODES_BUTTON, callback_data="promo_codes"
        ),
    ]
...
async def check_requests_callback(call: types.CallbackQuery, db_connection: aiosqlite.Connection):
    """Обработчик для проверки ожидающих запросов."""
    pending_requests = await get_pending_requests(db_connection)

    if pending_requests:
        response_text = AdminMessages.PENDING_REQUESTS_HEADER
        ...
        for user in pending_requests:
            ...
            markup = types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        types.InlineKeyboardButton(
                            text=AdminMessages.ACCEPT_BUTTON,
        ...
        back_markup = types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=AdminMessages.BACK_BUTTON, callback_data="admin_menu"
                    )
                ]
            ]
        )
        await call.message.answer(
            AdminMessages.PENDING_USERS_SHOWN, reply_markup=back_markup
...
async def accept_request_callback(call: types.CallbackQuery, state: FSMContext, db_connection: aiosqlite.Connection):
    ...
    try:
        ...
        await call.message.edit_text(
            AdminMessages.REQUEST_ACCEPTED.format(
                user_id=user_id, trial_days=trial_days
            ),
            parse_mode="HTML",
        )
        await send_sticker_and_message_with_cleanup(
            ...
            message_text=(enter_caption + "\n\n" + AdminMessages.ACCESS_GRANTED_MESSAGE),
            state=state,
        )
        await state.clear()
        await main_menu(user_id=user_id, db_connection=db_connection, state=state)  # Send main menu to the user
...
async def promo_codes_menu(call: types.CallbackQuery, state: FSMContext):
    ...
    buttons = [
        types.InlineKeyboardButton(
            text=AdminMessages.ADD_PROMO_BUTTON, callback_data="add_promo"
        ),
        types.InlineKeyboardButton(
            text=AdminMessages.LIST_PROMOS_BUTTON, callback_data="list_promos_menu"
        ),
        types.InlineKeyboardButton(
            text=AdminMessages.DELETE_PROMO_BUTTON, callback_data="delete_promo"
        ),
        types.InlineKeyboardButton(
            text=AdminMessages.BACK_BUTTON, callback_data="admin_menu"
        ),
    ...
    await call.message.edit_text(
        AdminMessages.PROMO_CODES_MENU,
        reply_markup=markup,
    )
    await state.clear()
...
async def add_promo_callback(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        AdminMessages.ADD_PROMO_PROMPT,
        parse_mode="HTML",
    )
...
async def process_promo_code_data(message: types.Message, state: FSMContext, db_connection: aiosqlite.Connection):
    ...
    if await add_promo_code(db_connection, code, days, usage_count):
        await message.answer(
            AdminMessages.PROMO_ADDED.format(
                code=code, days=days, usage_count=usage_count
            )
        )
    else:
...
async def list_promos_menu_callback(call: types.CallbackQuery, db_connection: aiosqlite.Connection):
    ...
    if promo_codes:
        ...
        for promo in promo_codes:
            response += AdminMessages.PROMO_LIST_ITEM.format(
                code=promo[0],
                days=promo[1],
                is_active="Да" if promo[2] == 1 else "Нет",
                usage_count=promo[3],
            )
    else:
...
async def process_promo_code_to_delete(message: types.Message, state: FSMContext, db_connection: aiosqlite.Connection):
    ...
    if await delete_promo_code(db_connection, promo_code):
        ...
        await message.answer(
            AdminMessages.PROMO_NOT_FOUND.format(promo_code=promo_code)
        )
    await state.clear()


@admin_router.message(Command("renewall"), IsAdmin())
async def renew_configs_handler(message: types.Message, state: FSMContext, db_connection: aiosqlite.Connection):
            ...
            if await update_user_configs(user_id, days):
                ...
                markup = types.InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            types.InlineKeyboardButton(
                                text=AdminMessages.HOME_MENU_BUTTON,
                ...
                await send_sticker_and_message_with_cleanup(
                    ...
                    message_text=AdminMessages.CONFIG_UPDATE_ATTENTION
    ...
    if failed_users:
        await bot.send_message(
            ADMIN_ID,
            AdminMessages.RENEW_ALL_FAIL.format(failed_users="\n".join(failed_users)),
        )
    else:
        await bot.send_message(
            ADMIN_ID,
            AdminMessages.RENEW_ALL_SUCCESS,
        )
...
async def process_user_id(message: types.Message, state: FSMContext, db_connection: aiosqlite.Connection):
...
async def process_broadcast_message(message: types.Message, state: FSMContext, db_connection: aiosqlite.Connection):
...
async def get_users_callback(call: types.CallbackQuery, db_connection: aiosqlite.Connection):
...
async def renew_access(message: types.Message, state: FSMContext, db_connection: aiosqlite.Connection):
    ...
    try:
        ...
        if not user:
            await message.reply(
                AdminMessages.USER_NOT_FOUND_IN_DB.format(user_id=user_id)
            )
            return
        ...
        if await update_user_configs(user_id, access_duration + 1):
            ...
            await send_sticker_and_message_with_cleanup(
                user_id=user_id,
                sticker_path="assets/warning.tgs",
                message_text=AdminMessages.SUBSCRIPTION_UPDATED_ATTENTION
                + AdminMessages.CONFIG_UPDATE_BODY
                + AdminMessages.ACCESS_ENDS_IN.format(
                    days=access_duration, day_word=get_day_word(access_duration)
                )
                + AdminMessages.REPLACE_CONFIG_IMPORTANT,
                state=state,
                markup=markup,
            ...
            await message.reply(
                AdminMessages.RENEW_SUCCESS.format(
                    ...
                    day_word=get_day_word(access_duration),
                )
            )
        else:
        ...
        logger.error(
            AdminMessages.UNEXPECTED_COMMAND_ERROR.format(
                user_id=message.from_user.id, e=e
            ),
            exc_info=True,
        )
...
async def update_access(message: types.Message, state: FSMContext, db_connection: aiosqlite.Connection):
    ...
    try:
        ...
        markup = types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(
                        text=AdminMessages.HOME_MENU_BUTTON, callback_data="main_menu"
                    )
                ],
            ]
        )
        await send_sticker_and_message_with_cleanup(
            user_id=user_id,
            sticker_path="assets/warning.tgs",
            message_text=AdminMessages.SUBSCRIPTION_EXTENDED_ATTENTION
            + f"Доступ к <b>УтяVPN</b> заканчивается через <b>{access_duration} {get_day_word(access_duration)}</b>.\n\n",
            state=state,
            markup=markup,
        )
        await message.reply(
...
async def refund_stars_handler(message: types.Message):
    ...
    try:
        ...
        await message.reply(
            AdminMessages.REFUND_SUCCESS.format(
                user_id=user_id, payment_charge_id=payment_charge_id
            )
        )
        logger.info(
        ...
        await message.reply(
            AdminMessages.UNEXPECTED_REFUND_ERROR.format(user_id=user_id, e=e)
        )
        logger.error(
